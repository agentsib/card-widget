...
QJ.data = (el, key, data = undefined) ->
  ...
  for element in elements
    if not element.customData then element.customData = {}
    if (data == undefined)
...
QJ.first = (el) ->
  ...
  if /NodeList\]/.test(el)
    ...
    elements = [el]
  return if elements.length then elements[0]
...
QJ.last = (el) ->
  ...
  if /NodeList\]/.test(el)
    ...
    elements = [el]
  return if elements.length then elements[elements.length - 1]
